...
async function getSettings() {
    try {
        const [settings] = await dbConnection.query('SELECT * FROM settings WHERE id = 1');
        if (settings.length > 0) {
            return settings[0];
        }
        // Return defaults if no settings exist
        return { targetCalories: 2000, limitCalories: 2500 };
    } catch (error) {
...
app.get('/', async function(req, res) {
    try {
        ...
        const [foodEntries] = await dbConnection.query(
            'SELECT * FROM food_entries ORDER BY dateTime DESC'
        );
...
// Settings page - process form
app.post('/settings', async function(req, res) {
    try {
        const { targetCalories, limitCalories } = req.body;
        
        // Check if settings exist
        const [existing] = await dbConnection.query('SELECT * FROM settings WHERE id = 1');
        
        if (existing.length > 0) {
            // Update existing settings
            await dbConnection.execute(
                'UPDATE settings SET targetCalories = ?, limitCalories = ? WHERE id = 1',
                [targetCalories, limitCalories]
            );
        } else {
            // Insert new settings
            await dbConnection.execute(
                'INSERT INTO settings (id, targetCalories, limitCalories) VALUES (1, ?, ?)',
                [targetCalories, limitCalories]
            );
        }
